...
public class CosmeticsManager {
	...
	public CosmeticsManager(SQLConnector con, Plugin plugin) {
		this.con = con;
		this.plugin = plugin;
	}
	...
	public void saveAll() {
		for(OfflinePlayer p : tokens.keySet()) {
			try {
				con.execute("UPDATE cosmetics SET TOKENS = " + tokens.get(p) + ", MEDALS = "+ goldMedals.get(p) + " WHERE UUID = '" + Utils.getUUID(p) + "';");
