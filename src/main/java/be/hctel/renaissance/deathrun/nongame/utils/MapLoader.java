...
public class MapLoader {
	...
	public void loadMaps() {
		...
		for(String name : worldNames) { 
			DeathRun.worldManager.loadWorld(name);
		}
	}
	public void loadWorldsToTempWorld(ArrayList<GameMap> map) {
		for(int i = 0; i < 6; i++) {
			DeathRun.worldManager.cloneWorld(map.get(i).getSystemName(), "TEMPWORLD" + i);
			DeathRun.worldManager.loadWorld("TEMPWORLD" + i);
	...
	public void deleteTempWorld() {
		for(int i = 0; i < 6; i++) {
			DeathRun.worldManager.deleteWorld("TEMPWORLD" + i);
		}
	}	
}
