...
import be.hctel.renaissance.deathrun.gamespecific.enums.GameMap;
import be.hctel.renaissance.deathrun.nongame.utils.ChatMessages;
...
public class VotesHandler {
	...
	public VotesHandler(Plugin plugin) {
		...
		new BukkitRunnable() {
			...
			@Override
			public void run() {
				DeathRun.mapLoader.loadWorldsToTempWorld(currentGameMaps);
	...
	public void sendMapChoices() {
		for(int i = 0; i < 5; i++) {
			TextComponent message = new TextComponent(DeathRun.header + "§7§l" + (i+1) + ". §6" + currentGameMaps.get(i).getMapName() + getVoteAmountString(currentGameMaps.get(i)));
		...
		TextComponent message = new TextComponent(DeathRun.header + "§7§l" + (6) + ". §cRandom Map" + getVoteAmountString(currentGameMaps.get(5)));
	...
	public void sendMapChoices(Player player) {
		player.sendMessage(DeathRun.header + "§e§lVote for a map! §7Use §f/v # §7or click.");
		for(int i = 0; i < 5; i++) {
			TextComponent message = new TextComponent(DeathRun.header + "§7§l" + (i+1) + ". §6" + currentGameMaps.get(i).getMapName() + getVoteAmountString(currentGameMaps.get(i)));
			message.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("§aClick to vote for §6" + currentGameMaps.get(i).getMapName()).create()));
			message.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/v " + (i+1)));
	...
	public void registerPlayerVote(Player player, int in, int qty) {
		if(!acceptingVotes) {
			player.sendMessage(DeathRun.header + ChatMessages.VOTESENDED.toText());
			return;
		}
		in--;
		if(in >= -1 && in < 6) {
			if(playerVote.containsKey(player)) {
				...
				if(playerVote.get(player) == in) {
					player.sendMessage(DeathRun.header + ChatMessages.ALREADYVOTED.toText());
				} else {
					votes.replace(currentGameMaps.get(playerVote.get(player)), votes.get(currentGameMaps.get(playerVote.get(player)))-qty);
					playerVote.replace(player, in);
					votes.replace(currentGameMaps.get(playerVote.get(player)), votes.get(currentGameMaps.get(playerVote.get(player))) + qty);
					if(in == 5) {
						player.sendMessage(DeathRun.header + ChatMessages.VOTEREGISTERED.toText() + "§cRandom Map" + getVoteAmountString(currentGameMaps.get(in)) + ".");
					} else {
						player.sendMessage(DeathRun.header + ChatMessages.VOTEREGISTERED.toText() + currentGameMaps.get(in).getMapName() + getVoteAmountString(currentGameMaps.get(in)) + ".");
					}
				}
			} else {
			...
			player.sendMessage(DeathRun.header + ChatMessages.NAN.toText() + " (1-6).");
		}
	}
	public int endVotes() {
